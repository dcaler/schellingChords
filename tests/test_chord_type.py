...
class TestChordImmutability:
    ...
    def test_cannot_modify_name(self, Chord):
        c = Chord(name="C", root=0, quality="major", pitch_classes=frozenset([0, 4, 7]), midi_voicing=(60, 64, 67))
...
class TestChordEqualityAndHash:
    ...
    def test_equal_content_same_hash(self, Chord):
        c1 = Chord(name="C", root=0, quality="major", pitch_classes=frozenset([0, 4, 7]), midi_voicing=(60, 64, 67))
        c2 = Chord(name="C", root=0, quality="major", pitch_classes=frozenset([0, 4, 7]), midi_voicing=(60, 64, 67))
        assert c1 == c2
        assert hash(c1) == hash(c2)

    def test_different_pitch_classes_unequal(self, Chord):
        c1 = Chord(name="C", root=0, quality="major", pitch_classes=frozenset([0, 4, 7]), midi_voicing=(60, 64, 67))
        c2 = Chord(name="F", root=5, quality="major", pitch_classes=frozenset([0, 5, 9]), midi_voicing=(65, 69, 72))
        assert c1 != c2

    def test_different_voicing_unequal(self, Chord):
        c1 = Chord(name="C", root=0, quality="major", pitch_classes=frozenset([0, 4, 7]), midi_voicing=(60, 64, 67))
        c2 = Chord(name="C", root=0, quality="major", pitch_classes=frozenset([0, 4, 7]), midi_voicing=(72, 76, 79))
        assert c1 != c2
...
class TestChordVoicingConsistency:
    ...
    @pytest.mark.parametrize("name,pitch_classes,voicing", [
        ("C", frozenset([0, 4, 7]), (60, 64, 67)),
        ("Dm", frozenset([2, 5, 9]), (62, 65, 69)),
        ("Em", frozenset([4, 7, 11]), (64, 67, 71)),
        ("F", frozenset([0, 5, 9]), (65, 69, 72)),
        ("G", frozenset([2, 7, 11]), (67, 71, 74)),
        ("Am", frozenset([0, 4, 9]), (69, 72, 76)),
        ("Bdim", frozenset([2, 5, 11]), (71, 74, 77)),
    ])
    def test_voicing_reduces_to_pitch_classes(self, Chord, name, pitch_classes, voicing):
        c = Chord(name=name, root=voicing[0] % 12, quality="major" if name in ("C", "F", "G") else ("minor" if name in ("Dm", "Em", "Am") else "diminished"),
...
class TestChordGoldenInstances:
    ...
    @pytest.mark.parametrize("name,root,quality,pitch_classes,voicing", [
        ("C", 0, "major", frozenset([0, 4, 7]), (60, 64, 67)),
        ("Dm", 2, "minor", frozenset([2, 5, 9]), (62, 65, 69)),
        ("Em", 4, "minor", frozenset([4, 7, 11]), (64, 67, 71)),
        ("F", 5, "major", frozenset([0, 5, 9]), (65, 69, 72)),
        ("G", 7, "major", frozenset([2, 7, 11]), (67, 71, 74)),
        ("Am", 9, "minor", frozenset([0, 4, 9]), (69, 72, 76)),
        ("Bdim", 11, "diminished", frozenset([2, 5, 11]), (71, 74, 77)),
    ])
